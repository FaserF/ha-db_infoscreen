...
async def test_past_departure_filtering(hass, mock_config_entry):
    ...
    sensor = DBInfoSensor(
        coordinator, mock_config_entry, "Karlsruhe Hbf", [], "", "", False
    )
...
async def test_zero_update_interval(hass, mock_config_entry):
    ...
    with patch(
        "custom_components.db_infoscreen.DBInfoScreenCoordinator.async_config_entry_first_refresh",
        AsyncMock(),
    ):
        ...
        assert (
            ...
            or coordinator.update_interval.total_seconds() == 0
        )
