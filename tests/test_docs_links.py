        ...
        for file_path in self.files:
            ...
            for line in content.splitlines():
                if line.startswith("#"):
                    ...
                    clean_line = re.sub(r'\{:\s*#[\w\-]+\s*\}', '', line).strip("#").strip()
                    if clean_line:
        ...
        for file_path in self.files:
            ...
            for link in links:
                ...
                if target_file and target_file != ".":
                    full_target_path = os.path.normpath(os.path.join(DOCS_DIR, target_file))
                ...
                if target_anchor:
                    ...
                    lookup_file = os.path.relpath(os.path.normpath(os.path.join(DOCS_DIR, target_file)), DOCS_DIR).replace("\\", "/")
                    ...
                    if target_anchor not in self.anchors[lookup_file]:
                        errors.append(f"Broken anchor link in {current_file}: {link} (Anchor #{target_anchor} not found in {lookup_file})")
